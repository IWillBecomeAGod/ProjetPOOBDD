...
namespace ProjetBDD {
	...
	private: System::Windows::Forms::Button^ button9;
		...
		MySqlCommand^ test()
		{
			...
			MySqlCommand^ cmdDataBase1 = gcnew MySqlCommand("INSERT INTO `livrableprojet`.`adresse` (`AdresseRue`, `AdresseVille`, `AdresseCodePostal`) VALUES ('"+rue+"', '" + ville + "', '" + CP + "');", conDataBase1);
			MySqlCommand^ cmdDataBase2 = gcnew MySqlCommand("SELECT `IDPersonnel` FROM `livrableprojet`.`personnel` WHERE `NomPersonnel` = '"+nomPersonnel+"' AND `PrenomPersonnel` = '"+prenomPersonnel+"'", conDataBase1);

			
			MySqlCommand^ cmdDataBase3 = gcnew MySqlCommand("INSERT INTO `livrableprojet`.`personnel` (`NomPersonnel`, `PrenomPersonnel`, `DateEmbauche`, `IDAdresse`, `IDPersonnel_est_supervise_par`) VALUES ('" + nomPersonnel + "', '" + prenomPersonnel + "', '" + dateEmbauche + "',"+ID+", 1); SELECT * FROM `livrableprojet`.`personnel`;", conDataBase1);
			...
			if (mode == "personnel")
			{
				if (action == "newadresse")
				{
					return cmdDataBase1;
				}
				if (action == "newsuperieur")
				{
					return cmdDataBase2;
				}
				if (action == "newpersonnel")
				{
					return cmdDataBase3;
				}
			}
...
private: System::Void button8_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (step == "encodeNewPersonnel")
	{
		nomSuperieur = textBox1->Text;
		prenomSuperieur = textBox2->Text;
		action = "newadresse";
...
private: System::Void button9_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (step == "superieur" && stepPrecedent == "adresse" && textBox1->Text!="")
	{
		stepPrecedent = "adresse";
		textBox1->Text = "";
		textBox2->Text = "";
